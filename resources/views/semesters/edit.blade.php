        ...
        <form method="POST" action="{{ route('semesters.update', $semester->id) }}">
            ...
            <div class="mb-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="course" name="course" value="{{ old('course', $semester->course) }}"
                        class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label for="batch" class="block text-sm font-medium mb-2">Batch</label>
                    <input type="text" id="batch" name="batch" value="{{ old('batch', $semester->batch) }}"
                        class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" id="semester_number" name="semester_number" 
                    value="{{ old('semester_number', $semester->semester_number) }}" min="1" max="10"
                    class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="notes" name="notes" rows="3"
                    class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('notes', $semester->notes) }}</textarea>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-2 sm:gap-4">
                ...
                <a href="{{ route('semesters.show', $semester->id) }}" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-6 rounded text-center">
                    Cancel
                </a>
            </div>
        </form>
